...
import {
  ...
  batalJanji,
  noShowJanji,
} from "./domain/aggregates/janji-root";
...
const main = () => {
  ...
  if (isFail(pasien)) {
    console.log("❌ Gagal daftar:", pasien.error);
    return;
  }
  ...
  if (isFail(janji)) {
    console.log("❌ Booking gagal:", janji.error);
    return;
  }
  ...
  if (isFail(konfirmasi)) {
    console.log("❌ Konfirmasi gagal:", konfirmasi.error);
    return;
  }
  ...
  if (isFail(mulai)) {
    console.log("❌ Mulai gagal:", mulai.error);
    return;
  }
  ...
  if (isFail(selesai)) {
    console.log("❌ Selesai gagal:", selesai.error);
    return;
  }
  ...
  const rm = catatRM(
    selesai.value,
    ["Miopia ringan", "Astigmatisme"],
    "Diberikan resep kacamata",
  );
  if (isFail(rm)) {
    console.log("❌ RM gagal:", rm.error);
    return;
  }
  ...
  if (isFail(janji2)) {
    console.log("❌ Booking 2 gagal:", janji2.error);
    return;
  }
  const batal = batalJanji(janji2.value, "Pasien sedang sakit");
  if (isFail(batal)) {
    console.log("❌ Batal gagal:", batal.error);
    return;
  }
  ...
  if (isFail(janji3)) {
    console.log("❌ Booking 3 gagal:", janji3.error);
    return;
  }
  const ulang = jadwalUlang(janji3.value, [], lusa, 480, 540);
  if (isFail(ulang)) {
    console.log("❌ Reschedule gagal:", ulang.error);
    return;
  }
  console.log(
    "✅ Janji dijadwalkan ulang — tgl:",
    ulang.value.tgl.toDateString(),
  );
  ...
  if (isFail(gagal))
    console.log("❌ (expected) Pasien non-aktif ditolak:", gagal.error);
  ...
  if (isFail(ulang1))
    console.log("❌ (expected) Reschedule 2x ditolak:", ulang1.error);

  // 12. NoShow
  const janji4 = bookingJanji(pasien.value, dokter, [], lusa, 660, 720);
  if (isFail(janji4)) {
    console.log("❌ Booking 4 gagal:", janji4.error);
    return;
  }
  const kf = konfirmasiJanji(janji4.value);
  if (isFail(kf)) {
    console.log("❌ Konfirmasi 4 gagal:", kf.error);
    return;
  }
  const ml = mulaiJanji(kf.value);
  if (isFail(ml)) {
    console.log("❌ Mulai 4 gagal:", ml.error);
    return;
  }
  const ns = noShowJanji(ml.value);
  if (isFail(ns)) {
    console.log("❌ NoShow gagal:", ns.error);
    return;
  }
